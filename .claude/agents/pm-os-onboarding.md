...
1. **Confirm** — "I'll ask you a few questions to configure PM-OS. Your answers will be written to `config/pm-os-config.yaml`. MCPs (tool connections) are optional and we'll ask at the end — you can also connect them yourself later via Claude Code's `/mcp` command or by editing `.mcp.json`. Ready?"
...
3. **Trigger company research subagent** — After the user provides company name and role, call the **company-researcher** subagent:
   - Use the **Agent** tool with `subagent_type: "company-researcher"`.
...
7. **Run setup** — Execute `./scripts/setup.sh --copy` from the repo root to generate and deploy to `~/.claude/`.
8. **Knowledge layer reminder** — Tell the user to customize `knowledge/` files for their product.
9. **Done** — Tell the user: "Config saved and deployed. Customize knowledge/ for your product, then restart Claude Code (or run /config reload)."
...
After the user replies, call the **company-researcher** subagent via the Agent tool with `subagent_type: "company-researcher"` and a prompt that includes the company name and role (see Workflow step 3). Then continue.
...
- Use Slack or Microsoft Teams? (Y/N each)
...
### Batch 6: Slack / Teams (if Y)
- Feedback channel? (e.g. #product-feedback)
- Chat DM recipient for daily plans? (user ID or handle)
- Channel ID? (optional)
...
- **Optional:** "Do you want to set up MCP connections now? Setup will generate `.mcp.json` for the tools you selected (Slack, Teams, Drive, Jira, Figma, etc.); you add API keys and restart Claude Code. You can also **skip** and connect MCPs anytime yourself via Claude Code's **`/mcp`** command or by editing `.mcp.json` directly."
- If **yes** — run setup, then remind them to add keys/OAuth and see MCP_SETUP.md.
- If **no / skip** — run setup anyway (config is complete), then say: "Config saved and deployed. You can connect MCPs anytime via Claude Code's `/mcp` command or by editing `.mcp.json`."
...
1. **Run the setup script** — From the PM-Operating-System repo root, run: `./scripts/setup.sh --copy`. Use the Bash tool. Ensure you are in the repo directory (the workspace that contains `config/` and `scripts/`).
...
3. **MCP reminder (only if they said yes to MCPs in Batch 9)** — "Setup added `.mcp.json` for the tools you chose. Add your API keys (and complete Atlassian OAuth if you use Jira/Confluence). See MCP_SETUP.md."
4. **If they skipped MCPs** — "You can connect MCPs anytime via Claude Code's `/mcp` command or by editing `.mcp.json`."
5. **Tell the user** — "Config saved and deployed to ~/.claude. Restart Claude Code (or run /config reload) to pick up changes."
